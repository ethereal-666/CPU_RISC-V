...
void malloc_run_state(RunState* s, Config* p) {
    ...
    if (!s->x || !s->xb || !s->xb2 || !s->hb || !s->hb2 || !s->q
     ...
     || !s->xq || !s->hq) {
        fatal_error("malloc_run_state failed");
    }
...
void quantize_int8(int8_t* q, float* f, int size, float* scale_out) {
    ...
    for (int i = 0; i < size; i++) {
        max_abs = fmaxf(max_abs, fabsf(f[i]));
    }
    *scale_out = max_abs > 0.0f ? max_abs / 127.0f : 1.0f;
    for (int i = 0; i < size; i++) {
        int val = (int)roundf(f[i] / *scale_out);
        if (val > 127) val = 127;
        if (val < -127) val = -127;
        q[i] = (int8_t)val;
    }
...
static int token_string_compare(const char *a, const char *b) {
    while (*a != '\0' && *a == *b) {
        a++;
        b++;
    }
...
int compare_tokens(const void *a, const void *b) {
    return token_string_compare(((const TokenIndex*)a)->str,
                                ((const TokenIndex*)b)->str);
}
...
void build_tokenizer(Tokenizer* t, /*char* tokenizer_path,*/ int vocab_size) {
    ...
    if (t->vocab == NULL || t->vocab_scores == NULL) {
        fatal_error("build_tokenizer failed");
    }
    ...
    for (int i = 0; i < vocab_size; i++) {       
        *(t->vocab_scores + i) = tokenizer_vocab_scores[i];
        ...
        if (t->vocab[i] == NULL) { fatal_error("token allocation failed"); }
...
char* decode(Tokenizer* t, int prev_token, int token) {
    if (token < 0 || token >= t->vocab_size) { return NULL; }
    char *piece = t->vocab[token];
    // following BOS (1) token, sentencepiece decoder strips any leading whitespace (see PR #89)
    if (prev_token == 1 && piece[0] == ' ') { piece++; }
    // careful, some tokens designate raw bytes, and look like e.g. '<0x01>'
    // Parse the exact <0xXX> byte-token form locally.  Pulling in sscanf on a
    // bare-metal target adds a large stdio dependency for this six-byte check.
    if (piece[0] == '<' && piece[1] == '0' &&
        (piece[2] == 'x' || piece[2] == 'X') &&
        piece[3] != '\0' && piece[4] != '\0' &&
        piece[5] == '>' && piece[6] == '\0') {
        int hi = (piece[3] >= '0' && piece[3] <= '9') ? piece[3] - '0' :
                 (piece[3] >= 'a' && piece[3] <= 'f') ? piece[3] - 'a' + 10 :
                 (piece[3] >= 'A' && piece[3] <= 'F') ? piece[3] - 'A' + 10 : -1;
        int lo = (piece[4] >= '0' && piece[4] <= '9') ? piece[4] - '0' :
                 (piece[4] >= 'a' && piece[4] <= 'f') ? piece[4] - 'a' + 10 :
                 (piece[4] >= 'A' && piece[4] <= 'F') ? piece[4] - 'A' + 10 : -1;
        if (hi >= 0 && lo >= 0) {
...
void encode(Tokenizer* t, char *text, int8_t bos, int8_t eos, int *tokens, int *n_tokens) {
    ...
    if (text == NULL) { fatal_error("cannot encode NULL text"); }

    if (t->sorted_vocab == NULL) {
        // lazily malloc and sort the vocabulary
        t->sorted_vocab = malloc(t->vocab_size * sizeof(TokenIndex));
        if (t->sorted_vocab == NULL) { fatal_error("token sort allocation failed"); }
    ...
    size_t str_capacity = t->max_token_length * 2 + 3;
    char* str_buffer = malloc(str_capacity * sizeof(char));
    if (str_buffer == NULL) { fatal_error("token buffer allocation failed"); }
    size_t str_len = 0;
    ...
    *n_tokens = 0;
    ...
    if (text[0] != '\0') {
        ...
        if (dummy_prefix < 0) { fatal_error("tokenizer has no space token"); }
    ...
    while (1) {
        ...
        for (int i=0; i < (*n_tokens-1); i++) {
            ...
            char *dst = str_buffer;
            char *limit = str_buffer + str_capacity - 1;
            while (*left != '\0' && dst < limit) { *dst++ = *left++; }
            while (*right != '\0' && dst < limit) { *dst++ = *right++; }
            *dst = '\0';
...
void build_sampler(Sampler* sampler, int vocab_size, float temperature, float topp, unsigned long long rng_seed) {
    ...
    if (sampler->probindex == NULL) { fatal_error("sampler allocation failed"); }
}
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    if (prompt_tokens == NULL) { fatal_error("prompt allocation failed"); }
    ...
    while (pos < steps) {
        ...
        if (token < 0 || token >= transformer->config.vocab_size) {
            fatal_error("token id out of range");
        }
        ...
        if (next < 0 || next >= transformer->config.vocab_size) {
            fatal_error("sampled token id out of range");
        }
...
int main(int argc, char *argv[]) {
    ...
    while (1)
    {
        ...
        char prompt[64] = {0};      // prompt string
#ifdef C_TEST
...
#ifdef C_TEST
        if (argc > 1) {
            strncpy(prompt, argv[1], sizeof(prompt) - 1);
...
#else
        int prompt_length = uart_readline(prompt, (int)sizeof(prompt));
        if (prompt_length == -2) {
            printf("Prompt is too long (maximum 63 bytes)\n");
            continue;
        }
        if (prompt_length <= 0) {
            continue;
